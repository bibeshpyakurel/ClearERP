...
| Cloudflare Pages | Frontend hosting (static SPA) |
...
| Frontend | [Cloudflare Pages](https://pages.cloudflare.com) | Static build, global CDN, no cold start |
...
1. In Cloudflare, choose **Workers & Pages → Create → Pages** and connect this
   repository.
...
4. Deploy, then note the assigned URL, e.g. `https://clearerp.pages.dev`.

[`frontend/public/_redirects`](frontend/public/_redirects) handles SPA routing so
that deep links like `/purchase-orders/42` resolve to `index.html` instead of a
404.

### 4. Connect the two

Set `CORS_ALLOWED_ORIGIN` on the Render service to the Pages URL and redeploy:

```
CORS_ALLOWED_ORIGIN = https://clearerp.pages.dev
```

The value accepts a comma-separated list. Cloudflare gives every branch and
preview build its own subdomain, so add any preview origins you actually use —
they are distinct origins and will otherwise fail CORS:

```
CORS_ALLOWED_ORIGIN = https://clearerp.pages.dev,https://dev.clearerp.pages.dev
```
...
| `VITE_API_BASE_URL` | Cloudflare Pages | API base URL including `/api` |
